...
- [ ] รันสคริปต์ backup แล้วได้ไฟล์ dump/tar.gz จริง ไม่ error
- [ ] **ทดสอบ restore บนฐานข้อมูลใหม่ (คนละตัวกับของจริง)** แล้วข้อมูลครบถ้วนตรงกับต้นฉบับ — ต้องทำจริง ไม่ใช่แค่เชื่อว่าสคริปต์ทำงาน
- [ ] ทดสอบ restore ไฟล์แนบแล้วเปิดไฟล์ได้ปกติ ไม่เสียหาย
- [ ] ทดสอบ retention policy: ไฟล์ backup ที่เก่ากว่า 30 วันถูกลบจริงตามกำหนด
- [ ] จำลองสถานการณ์ "เครื่อง server เสียหายทั้งหมด" แล้วทำตาม `docs/runbooks/disaster-recovery.md` กู้ระบบขึ้นมาใหม่บนเครื่องอื่นได้สำเร็จ (ทดสอบอย่างน้อย 1 ครั้งก่อนส่งมอบโครงงาน)
