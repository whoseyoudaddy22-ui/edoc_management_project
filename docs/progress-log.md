...
- [x] Revoke GitHub PAT ที่ใช้ push หลายรอบ (หลุดเข้า chat history ไปแล้ว) แล้วเปลี่ยนไปใช้ `gh auth login` แทนในอนาคต (2026-07-11) — revoke ที่ GitHub settings แล้ว
...
- [x] Revoke GitHub PAT เก่า (2026-07-11)
...
แก้บั๊ก production จริง: dropdown "ประเภทเอกสาร" ในหน้าสร้างเอกสารว่างเปล่า ใช้เลือกอะไรไม่ได้เลย

- **หา root cause:** ตรวจ `documents/create/page.tsx` พบว่า dropdown ดึงจากตาราง `DocumentType` (`isActive: true`) ตรงๆ — สั่งให้ผู้ใช้รัน `docker exec docs-db psql ... SELECT * FROM "DocumentType"` บน production จริงแล้วยืนยันได้ **0 rows** สาเหตุคือไม่เคยมี seed script ไหนใส่ข้อมูลนี้ลง production เลยตั้งแต่ต้นโปรเจกต์ (`seed-prod.ts` เดิม seed แค่ user, ส่วน `seed-documents.ts` ที่มีรายการ 5 ประเภทเป็น dev-only fixture สำหรับสร้างเอกสารทดสอบ 120 รายการ ไม่ควรรันกับ prod)
- **แก้ใน `prisma/seed-prod.ts`** (commit `2ee291d`): เพิ่ม `upsert` 5 ประเภทเอกสารมาตรฐาน (รหัสอ้างอิงจาก `.claude/skills/document-numbering/SKILL.md`) ปลอดภัย รันซ้ำได้ ไม่ทับ/ลบข้อมูลเดิม
- **เจอปัญหาต่อระหว่างพยายามรัน seed ผ่าน `docker compose run --rm app npx tsx prisma/seed-prod.ts`:** `MODULE_NOT_FOUND` ที่ `../src/lib/prisma` เพราะ image ของ service `app` (production runtime) copy มาแค่ `src/generated` เท่านั้น (ดู `Dockerfile` stage `runner`) ไม่มี `src/lib` ที่ script นี้ import อยู่ — เป็นช่องโหว่เดิมของ infra ที่ไม่เคยถูกใช้งานมาก่อน เพราะรอบก่อนหน้าที่เคย seed user บน production (2026-07-06) ทำตอนที่ยังรัน `next start` ตรงบน host ก่อนย้ายมา `docker compose` เต็มรูป
- **แก้ด้วยการเพิ่ม build stage ใหม่** (commit `10b2882`): stage `tools` ใน `Dockerfile` (full `node_modules` + source + `prisma generate`, ไม่ prune devDependencies) และ service `seed` ใหม่ใน `docker-compose.yml` ที่ build จาก stage นี้ (`profiles: ["tools"]` เหมือน `migrate` เดิม ไม่ auto-start ตอน `up` ปกติ) — build image ทดสอบผ่านจริงบนเครื่อง dev (`docker build --target tools`, exit code 0) ก่อน push
- **Pre-merge checklist ผ่านครบก่อน merge:** `npm run test` 95/95 ผ่าน, security review เฉพาะ diff (3 ไฟล์ ไม่มี user input ใหม่, `.dockerignore` กัน `.env*` ออกจาก build context ของ stage `tools` อยู่แล้ว ไม่มี secret รั่วเข้า image)
- **เปิด PR #1 → merge เข้า `master`** (merge commit `1246d27`) — ระหว่างนี้พบว่าเครื่องนี้ไม่เคยติดตั้ง `gh` CLI เลย ติดตั้งผ่าน `winget install GitHub.cli` แล้วให้ผู้ใช้รัน `gh auth login` เอง (web browser flow) ก่อนเปิด PR/merge ได้
- **Deploy ขึ้น production สำเร็จ** ตาม flow เดิม: `git pull` → `docker compose --env-file .env.production run --rm migrate` (ผ่าน ไม่มี migration ใหม่จาก PR นี้เอง) → `docker compose --env-file .env.production up -d --build app` → log สะอาด, container `Ready in 145ms`
- **ยืนยันจากผู้ใช้จริงบน production:** เลือกประเภทเอกสารในหน้าสร้างเอกสารได้แล้ว ปิดเคสนี้สมบูรณ์
- **ผลพลอยได้ที่เก็บไว้ใช้ต่อ:** service `seed` ใหม่นี้เป็น pattern มาตรฐานสำหรับ seed script ใดๆ ในอนาคตที่ต้อง import จาก `src/lib` ตรงๆ ไม่ต้องแก้ infra ซ้ำอีกครั้ง

## 2026-07-13

แก้บั๊กหน้าจอบนมือถือ: ผู้ใช้ส่งภาพหน้าจอจากโทรศัพท์จริง (`https://192.168.1.155`) มาให้ดู — sidebar เมนูแสดงเต็มความสูงกินพื้นที่จอไปกว่าครึ่ง เนื้อหาแดชบอร์ดถูกบีบไปอยู่คอลัมน์แคบทางขวาจนอ่านยาก

- **หา root cause:** `src/app/(dashboard)/layout.tsx` render `Sidebar` (`w-64` ตายตัว แสดงตลอด ไม่มี toggle) คู่กับ `<main>` padding `p-6` ตายตัว — บนจอมือถือ (~375px) พื้นที่เหลือให้เนื้อหาแทบไม่มี ทุกหน้าใต้ `(dashboard)` โดนเหมือนกันหมดเพราะใช้ layout เดียวกัน — ตรงกับที่ `.claude/skills/ui-design-system/SKILL.md` เขียนไว้เดิมว่า "ไม่ยุบ/ขยายในเวอร์ชันแรก" (ตั้งใจแบบนั้นตอนแรกเพราะกลุ่มผู้ใช้เดิมใช้คอมพิวเตอร์เป็นหลัก)
- **แก้ผ่าน plan mode** (commit `ea17797`, merge เข้า master ที่ `c5ec345` ผ่าน PR #2): แยก `sidebar.tsx` เป็น `SidebarContent` (ใช้ซ้ำได้) + `Sidebar` (wrapper desktop, `hidden lg:flex`) เพิ่ม `dashboard-shell.tsx` ใหม่ (mobile top bar + hamburger เปิด shadcn `Sheet` drawer จาก `npx shadcn add sheet` อิง `@base-ui/react/dialog`) ครอบ `SidebarContent` เดียวกัน ปิดอัตโนมัติเมื่อ tap เมนู — อัปเดต `ui-design-system` skill คู่กับโค้ด (Sidebar/Responsive Rules section + convention ใหม่ "Mobile Navigation (Drawer)" + changelog v2.7) ตามกติกาของโปรเจกต์ที่ห้ามมี pattern UI นอกเหนือจาก skill
- **เจอบั๊กแทรกระหว่างทาง (ไม่เกี่ยวกับงานนี้) — CSP บล็อก hydration บน dev server:** `npm run dev:ui-test` (port 3001) กดอะไรไม่ได้เลยทั้งหน้า เพราะ `next.config.ts` มี CSP `script-src` ไม่มี `'unsafe-eval'` ซึ่ง React/Turbopack dev mode ต้องใช้ตอน HMR — ลองแก้แบบ dev-only (`'unsafe-eval'` เฉพาะ `NODE_ENV !== "production"`) แต่โดน auto-mode classifier บล็อกเพราะเป็นการแก้ security control นอกขอบเขตงานที่อนุมัติไว้ — **revert คืนแล้ว ไม่ได้แก้ บั๊กนี้ยังอยู่** ให้ผู้ใช้ตัดสินใจเองว่าจะแก้ตอนไหน (ทางเลือกที่เสนอไว้: อนุญาตแก้ CSP เฉพาะ dev / ยอมรับว่า verify ผ่าน production build แทนตลอดไป)
- **วิธี verify ที่ใช้แทน:** build โหมด production จริงบนเครื่อง dev เอง (`npx dotenv-cli -e .env.test -- npx next start -p 3002`, ยืนยันด้วย `node -e` ว่า `DATABASE_URL` ชี้ไปที่ `docs_management_test` ก่อนรันเสมอ ไม่แตะ prod) เพราะ production build ไม่ต้องพึ่ง `eval()` เลยไม่เจอบั๊กนี้ — เจอ credential ผิดระหว่างทางด้วย (`saraban@test.com` เป็นบัญชีของ `seed-advisor.ts` สำหรับ dev/prod เท่านั้น ไม่เคย seed ลง `docs_management_test`; บัญชีที่ใช้ได้จริงคือ `saraban-test@organization.go.th` / `test1234` จาก `seed-test.ts`) และเจอ migration ค้างอีกรอบ (`docs_management_test` ยังไม่มี `sessionInvalidatedAt`) แก้ด้วย `npm run test:db:migrate`
- **ไล่ตรวจทุกหน้าใต้ `(dashboard)` จริงที่ 375px/768px/1280px** (dashboard, เอกสารทั้งหมด, สร้างเอกสาร, รายละเอียดเอกสาร, พิมพ์เอกสาร, อัปโหลดไฟล์, จัดการผู้ใช้งาน+เพิ่มผู้ใช้, ประวัติการใช้งานระบบ, สำรองข้อมูล) — ไม่พบ horizontal overflow เพิ่มเติมนอกจาก sidebar/layout เดิม (หน้าอื่นที่เคยทำ responsive ไว้บ้างแล้วจากรอบ redesign ก่อนหน้าใช้งานได้ปกติทันทีที่ sidebar ไม่กินพื้นที่แล้ว) ยืนยัน drawer เปิด/ปิด/navigate ถูกต้อง, desktop (`lg`+) ไม่มี regression เลย
- **Pre-merge checklist ผ่านครบ:** `npm run test` 95/95, typecheck สะอาด (เหลือแค่ error เดิมใน test files ที่ไม่เกี่ยวข้อง), security review เฉพาะ diff (5 ไฟล์ ไม่แตะ auth/API/data logic เลย — `layout.tsx` เช็ค session เหมือนเดิมทุกประการ แค่เปลี่ยนวิธี render)
- **Deploy ขึ้น production สำเร็จ**: `git pull` → `docker compose --env-file .env.production run --rm migrate` (ไม่มี migration ใหม่จาก PR นี้) → `up -d --build app` — **ผู้ใช้ยืนยันจากโทรศัพท์จริงแล้วว่าแสดงผลถูกต้อง** ปิดเคสนี้สมบูรณ์
...
### การตัดสินใจ: ไม่ไล่ทำ TODO ที่เหลือต่อ (บ่าย 2026-07-28)

ผู้ใช้ชี้แจงว่าโปรเจกต์นี้เป็น**โครงงานจบการศึกษา ไม่ใช่ระบบ production ที่ใช้งานจริงต่อเนื่อง** จึงตัดสินใจไม่ไล่แก้รายการ hardening ที่เหลือ (คุ้มค่าสำหรับระบบที่ต้องรันระยะยาวมีผู้ใช้จริง แต่ไม่กระทบการสาธิต/ให้คะแนนโครงงาน):

- [ ] ~~บั๊ก CSP บล็อก `eval()` บน dev server~~ — กระทบแค่ dev workflow ส่วนตัว ไม่กระทบแอปที่ deploy จริง ปล่อยไว้ ใช้ production build workaround ต่อไปถ้าจำเป็น
- [ ] ~~ตั้ง DHCP reservation ที่ router~~ — ป้องกันความเสี่ยงระยะยาวที่ router แจก IP ซ้ำ ไม่ใช่ประเด็นสำหรับช่วงทำโครงงาน
