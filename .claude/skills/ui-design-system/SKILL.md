...
| 3. ปรับหน้าตาม module (เริ่ม Login / Module 16) | ✅ Login (Module 16), Dashboard, Sidebar Navigation, เอกสารทั้งหมด, ผู้ใช้งาน (ฟอร์มเพิ่ม/แก้ไข) เสร็จแล้ว (2026-07-08) — โมดูลถัดไปยังไม่ทำ (สร้างเอกสาร, อัปโหลดไฟล์, พิมพ์เอกสาร, ประวัติการใช้งาน, สำรองข้อมูล, ตาราง/badge จัดการผู้ใช้งานเอง [`users-table.tsx`, `role-badge.tsx`] ยังไม่ได้แตะ) |
...
- **Sidebar:** ความกว้างคงที่ `w-64` แสดงตลอดเฉพาะจอ `lg` ขึ้นไป (`hidden lg:flex`) — ต่ำกว่านั้นซ่อนแล้วเปิดผ่าน off-canvas drawer แทน (ดู "Mobile Navigation (Drawer)" ด้านล่าง)
- **แบ่งเมนู sidebar เป็นกลุ่ม** พร้อมหัวข้อกลุ่มตัวเล็ก (เช่น "ภาพรวม", "เอกสาร", "ระบบ") ตาม mockup
- **พื้นที่เนื้อหาหลัก:** padding รอบนอก `p-4 sm:p-6` (แคบลงบนจอมือถือ), ระยะห่างระหว่าง section `gap-6`, wrapper ของ `<main>` ต้องมี `min-w-0` เสมอ (กัน flex child ไม่ยอมหดตามเนื้อหากว้าง ทำให้ทั้งหน้า overflow แนวนอน)
...
เพิ่มเข้ามารอบปรับ responsive (v2.7) — ทุกหน้าใต้ `(dashboard)` ใช้ `src/components/shared/dashboard-shell.tsx` เป็น shell ครอบ ไม่ใช่ `Sidebar` + `<main>` ตรงๆ อีกต่อไป

- เนื้อหาเมนู (โลโก้/ชื่อระบบ, กลุ่มเมนู, user footer+logout) อยู่ที่ `SidebarContent` (export จาก `sidebar.tsx`) ใช้ร่วมกันทั้ง 2 ที่ — **ห้าม duplicate รายการเมนูไว้คนละที่** ถ้าจะเพิ่ม/แก้เมนูใหม่ในอนาคต ให้แก้ที่ `NAV_GROUPS`/`ADMIN_NAV_GROUP` ใน `sidebar.tsx` จุดเดียว ทั้ง desktop sidebar และ mobile drawer จะอัปเดตตามกันเอง
- Desktop (`lg` ขึ้นไป): `Sidebar` wrapper (`hidden lg:flex`) เรียก `SidebarContent` ตรงๆ พฤติกรรมเดิมทุกประการ
- มือถือ/แท็บเล็ต (ต่ำกว่า `lg`): `dashboard-shell.tsx` render top bar บาง (`lg:hidden`, สี `bg-sidebar text-sidebar-foreground`) มีปุ่มแฮมเบอร์เกอร์ (ไอคอน `Menu` จาก `lucide-react`) เปิด shadcn `Sheet`/`SheetContent side="left"` (component `src/components/ui/sheet.tsx` มาจาก `npx shadcn add sheet`, อิง `@base-ui/react/dialog`) ครอบ `SidebarContent` เดียวกัน — สี `SheetContent` ต้อง override เป็น `bg-sidebar text-sidebar-foreground` (ไม่ใช่ default `bg-popover`)
- `SidebarContent` รับ prop `onNavigate?: () => void` เรียกตอนกด `<Link>` ในเมนู เพื่อปิด drawer อัตโนมัติหลัง navigate — ทุกจุดที่ใช้ `SidebarContent` ในบริบท drawer ต้องส่ง prop นี้เสมอ (desktop ไม่ต้องส่ง เพราะไม่มี drawer ให้ปิด)
- `Sheet` มี focus trap/ESC-to-close/click-backdrop-to-close ให้จาก base-ui Dialog primitive อยู่แล้ว ไม่ต้องเขียนเอง
...
- Breakpoint หลักตาม Tailwind default (`sm`, `md`, `lg`, `xl`) — sidebar/drawer สลับกันที่ `lg` (1024px)
- ตั้งแต่รอบ v2.7 (2026-07-13) ทุกหน้าใต้ `(dashboard)` ต้อง **ใช้งานได้จริงบนมือถือ** ไม่ใช่แค่ "ไม่พัง" เฉยๆ — ต้องไม่มี horizontal overflow ที่ `<body>` (ยกเว้น container ที่ตั้งใจให้ scroll เอง เช่นตาราง), ปุ่ม/ฟอร์มกดใช้งานได้จริงบนจอแคบ ทดสอบด้วย viewport 375px (มือถือ) และ 768px (แท็บเล็ต) ก่อน merge เสมอ
- หน้า print/export (`documents/[id]/print`) ยกเว้นได้ ปล่อยเป็น fixed A4 layout ตามเจตนา (ไว้พิมพ์จริง) แต่ต้องไม่ทำให้แอปพังเวลาเข้าจากมือถือ
...
- **v2.7 (2026-07-13):** ปรับ layout หลักให้ responsive จริงบนมือถือ — sidebar (`w-64` คงที่) เดิมพังบนจอแคบเพราะแสดงตลอดไม่มี toggle ทำให้พื้นที่เนื้อหาเหลือแทบไม่มี แก้โดยเพิ่ม `src/components/shared/dashboard-shell.tsx` (mobile top bar + hamburger เปิด shadcn `Sheet` drawer จาก `npx shadcn add sheet`) และแยก `sidebar.tsx` เป็น `SidebarContent` (ใช้ซ้ำได้) + `Sidebar` (wrapper desktop `hidden lg:flex`) ดู convention ใหม่ "Mobile Navigation (Drawer)" — ไล่ตรวจทุกหน้าใต้ `(dashboard)` ที่ 375px/768px/1280px แล้วไม่พบ overflow เพิ่มเติมนอกจาก sidebar/layout (หน้าอื่นที่เคยทำ responsive ไว้บ้างแล้ว เช่น dashboard grid, ตารางเอกสาร/ผู้ใช้งาน/audit log ที่มี `overflow-x-auto`, ฟอร์มสร้างเอกสาร/ผู้ใช้งานที่เป็น `md:grid-cols-2` ใช้งานได้ปกติหลัง sidebar ไม่กินพื้นที่แล้ว) — เปลี่ยน Responsive Rules จาก "ไม่จำเป็นต้อง mobile-first" เป็นต้องใช้งานได้จริงบนมือถือตั้งแต่รอบนี้เป็นต้นไป
- **v2.6 (2026-07-08):** ปรับฟอร์ม "เพิ่มผู้ใช้งาน"/"แก้ไขผู้ใช้งาน" (`user-create-form.tsx`, `user-edit-form.tsx`) ให้ข้อมูลครบขึ้น — แยก `name` เดิมเป็น `titlePrefix` (dropdown นาย/นาง/นางสาว) + `firstName` + `lastName` (ยังคง `name` ไว้เป็นชื่อเต็มคำนวณอัตโนมัติ ไม่กระทบจุดอื่นที่ยังอ้าง `user.name` อยู่) เพิ่ม `division` (dropdown ฝ่ายบริหาร/ทรัพยากรบุคคล/การเงินและบัญชี/การตลาด/ขาย/ไอที/บริการลูกค้า) และ `position` (ตำแหน่งงาน ข้อความอิสระ) — **ข้อควรระวังสำคัญ:** `division` (ฝ่าย) เป็นคนละฟิลด์กับ `departmentCode` (หน่วยงาน) ที่มีอยู่เดิม ห้ามสับสน/รวมกัน เพราะ `departmentCode` คือรหัสย่อ 2-4 ตัวอักษรที่ระบบเลขที่เอกสารอัตโนมัติใช้เป็น prefix (เช่น `ศรพ`, `ทสบ`) ส่วน `division` เป็นข้อมูลทำเนียบพนักงานล้วนๆ ไม่เกี่ยวกับเลขที่เอกสาร — ฟอร์มจึงแสดงทั้งสองฟิลด์แยกกันพร้อม helper text กำกับใต้ "หน่วยงาน" ฟิลด์ใหม่ทั้งหมด (`titlePrefix`/`firstName`/`lastName`/`division`) เป็น optional ที่ระดับ database (nullable) เพื่อไม่กระทบบัญชีเดิมที่มีอยู่ก่อน migration แต่ฟอร์มบังคับกรอกสำหรับผู้ใช้งานใหม่ (มี `*`) ส่วนหน้าฟอร์มเองก็ผ่านการทำความสะอาด palette class ดิบ (`text-gray-900`, `text-red-500/600`, `bg-red-50` ฯลฯ) เป็น token ระหว่างที่แก้ไปด้วย
- **v2.5 (2026-07-08):** ปรับหน้า "เอกสารทั้งหมด" (`documents-table.tsx` + `advanced-search-panel.tsx`) เสร็จ — ลบ palette class ดิบ (`text-gray-*`, `bg-black/10`, `bg-red-50`/`text-red-700`/`border-red-200`, `bg-white`, `accent-blue-600`) เปลี่ยนเป็น token (`text-foreground`, `text-muted-foreground`, `bg-foreground/10`, `bg-card`) และแก้ checkbox ทั้ง 5 กลุ่มใน `AdvancedSearchPanel` (ประเภทเอกสาร/สถานะ/ความเร่งด่วน/ผู้สร้าง/หน่วยงาน) จาก `<input type="checkbox">` ดิบเป็น shadcn `Checkbox` ให้ตรงกติกา Form convention ที่มีอยู่แล้ว (checkbox ทุกตัว = shadcn Checkbox) และเปลี่ยน error banner ของตารางจาก `<p>` แต่งเองเป็น `Alert` variant `destructive` ให้ตรงมาตรฐานเดียวกับหน้า Login
