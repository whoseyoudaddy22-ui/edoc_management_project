...
## การพิมพ์เลขที่ด้วยมือ (Manual Override)

ผู้ใช้พิมพ์เลขที่หนังสือเองแทนเลขที่ระบบออกให้อัตโนมัติได้เสมอ (ไม่ล็อกเฉพาะเอกสารแรก) — ใช้กับ
ทุกประเภทเอกสาร กรณีหลักคือนำเข้าเลขที่จริงจากหน่วยงานต้นทาง (เช่น `พล 0104/25`) ซึ่ง **ไม่ต้องตรง
กับ pattern มาตรฐาน** ในหัวข้อ "รูปแบบเลขที่เอกสาร" ด้านบนก็ได้

**สิ่งที่ยังทำงานเหมือนเดิมเสมอ ไม่ว่าจะพิมพ์เองหรือไม่:**

- `buddhistYear`/`runningNumber` คำนวณอัตโนมัติทุกครั้งที่สร้างเอกสาร (แม้ `documentNumber` ที่บันทึกจริงจะเป็นค่าที่ผู้ใช้พิมพ์เอง) เพื่อให้เลขแนะนำของเอกสารถัดไปในหน่วยงาน+ประเภทเดียวกันยังต่อเนื่องเป็นรูปแบบมาตรฐานได้
- Regex validation (`^[ก-๙]{2,4}\.\d{4}\/\d{4}-\d{3}$`) ใช้ตรวจ/แสดงผลเฉพาะเลข **ที่ระบบ generate เอง** เท่านั้น ไม่บังคับกับค่าที่ผู้ใช้พิมพ์เข้ามา
- `documentNumber` ยังคง unique ระดับ database เสมอ — พิมพ์เลขซ้ำกับที่มีอยู่แล้วต้อง reject ทันที (409) ไม่ retry เหมือนกรณีเลขชนกันจาก concurrency (ดู `DuplicateDocumentNumberError` ใน `src/lib/document-number.ts`)

### Flow

1. ฝั่ง client เรียก `GET /api/documents/next-number?documentTypeCode=...` เพื่อขอเลข "แนะนำ" (รูปแบบมาตรฐาน) มาเติมเป็นค่าเริ่มต้นในช่อง "ที่" ของฟอร์มสร้างเอกสาร — endpoint นี้แค่ preview ไม่ได้จองเลข
2. ผู้ใช้แก้ไขค่านี้ทับได้อิสระ (พิมพ์เลขจริงจากหน่วยงานต้นทาง)
3. ตอน `POST /api/documents` ถ้า client ส่ง `documentNumber` มา (แปลว่าผู้ใช้แก้ไขค่าที่เสนอ) `createDocumentWithAutoNumber` จะใช้ค่านั้นเป็น `documentNumber` ที่บันทึกจริง แต่ยังคำนวณ `buddhistYear`/`runningNumber` แบบ atomic เหมือนเดิมทุกประการ (ดู `manualDocumentNumber` parameter)
4. ถ้า client ไม่ส่ง `documentNumber` มา (ปล่อยตามค่าที่ระบบเสนอโดยไม่แก้ไข) ระบบจะคำนวณเลขมาตรฐานให้ใหม่แบบ atomic ตอนบันทึกจริงเสมอ (ไม่ใช้ค่าจาก preview endpoint ตรงๆ) เพื่อกันrace condition ระหว่างช่วงเวลาที่ขอ preview กับตอนบันทึกจริง
...
- [ ] ทดสอบ validation regex กับ input ที่ผิดรูปแบบ (เฉพาะเลขที่ระบบ generate เอง) ต้อง reject ทุกกรณี
- [ ] พิมพ์เลขที่เองแบบอิสระ (ไม่ตรง pattern มาตรฐาน) แล้วสร้างเอกสารสำเร็จ และเอกสารถัดไปในหน่วยงาน+ประเภทเดียวกันยังได้เลขแนะนำแบบมาตรฐานต่อเนื่องถูกต้อง
- [ ] พิมพ์เลขที่ซ้ำกับเอกสารที่มีอยู่แล้ว ต้อง reject ทันทีด้วย error message ที่ชัดเจน ไม่ retry ค้าง
